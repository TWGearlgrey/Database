...
INSERT INTO `Patients` (`pat_id`, `doc_id`, `nur_id`, `pat_name`, `pat_jumin`, `pat_gen`, `pat_addr`, `pat_phone`, `pat_job`) 
		VALUES ('P102103', 'D102104', 'N102102', '유재석', '721128-1234567', 'M', '부산', '010-6601-7211', '개그맨');
INSERT INTO `Patients` (`pat_id`, `doc_id`, `nur_id`, `pat_name`, `pat_jumin`, `pat_gen`, `pat_addr`, `pat_phone`, `pat_job`) 
...
INSERT INTO `Nurses` VALUES ('N108101', '송중기', '1992-10-07', 'M', 108, '간호사',   '010-7303-9210', 'jungi@bw.com');
...
#문제1. 모든 의사의 의사id 이름 생년월일 전공의과번호 전공의과 이름을 조회하십시오
SELECT
	`doc_id`,
	`doc_name`,
	`doc_birth`,
...
#문제2. 모든 간호사의 간호사id 이름 생년월일 전공의과번호 전공의과이름을 조회하십시오
SELECT 
	`nur_id`,
	`nur_name`,
	`nur_birth`,
...
#문제3. 모든 환자의 환자id 이름 주민번호 휴대폰 담당의사이름 담당간호사 이름을 조회하십시오
SELECT
	`pat_id`,
	`pat_name`,
	`pat_jumin`,
...
#문제4. 모든 진료내역에서 환자이름, 담당의사이름, 진료내용, 처방내용, 진료날짜를 조회하시오.
SELECT
	`pat_name`,
	`doc_name`,
	`treat_contents`,
...
FROM `Treatments` AS a
JOIN `Doctors`    AS b ON a.doc_id = b.doc_id
JOIN `Department` AS c ON b.dep_no = c.dep_no
...
#문제9 김태희 간호사가 맡고 있는 모든 환자의 이름을 구분자 ^으로 해서 조회하시오
SELECT 
	GROUP_CONCAT(`pat_name` SEPARATOR '^') AS `김태희 간호사 담당 환자`
FROM `Patients` AS a
JOIN `Nurses` AS b ON a.nur_id = b.nur_id
WHERE `nur_name`='김태희';


#문제10 가장 많은 환자 수를 담당하는 간호사 id, 간호사 이름, 담당 환자수를 조회하시오
SELECT
	a.`nur_id`,
	`nur_name`,
	COUNT(*) AS `담당 환자수` 
