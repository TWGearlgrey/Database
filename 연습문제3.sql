...
FROM `Student` AS a JOIN `Register` 
					AS b ON a.stdNo = b.regStdNo
WHERE `regLecNo` = 349;
...
JOIN `Register` AS b ON a.stdNo = b.regStdNo
JOIN `Lecture`  AS c ON b.regLecNo = c.lecNo
WHERE `regGrade` != 'F' #대신 WHERE `regTotalScore >= 60을 해도 됨.
GROUP BY `stdNo`
ORDER BY `stdNo`;
...
JOIN `Register` AS b ON a.stdNo = b.regStdNo
JOIN `Lecture`  AS c ON b.regLecNo = c.lecNo 
GROUP BY `stdNo`; #WHERE을 쓰지 않는 이유는 if문 안에 이미 조건이 들어가 있기 때문.
	#Group_CONCAT 컬럼의 문자열을 연결해줌
